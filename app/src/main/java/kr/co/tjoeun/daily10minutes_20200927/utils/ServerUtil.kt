...
//            임시 확인 : 어떻게 url이 완성되었는지 확인
        ...
        fun getRequestNotiCount(context:Context, handler: JsonResponseHandler?) {
...
//            임시 확인 : 어떻게 url이 완성되었는지 확인
        ...
        fun getRequestProjectProofListByIdAndDate(context:Context, projectId:Int, date:String, handler: JsonResponseHandler?) {
...
//            Intent 덩어리에 대응되는 개념.
            ...
            val request = Request.Builder()
                .url(urlString)
                .post(formData)
                .header("X-Http-Token", ContextUtil.getLoginUserToken(context)) // API가 header 데이터를 요구하면 담아주는 곳
                .build()
...
//                    그 가이드북에 적힌 내용을 실제로 실행해달라는 코드.
        ...
        fun postRequestWriteProofReply(context:Context, proofId: Int, content:String, handler: JsonResponseHandler?) {
...
//            로그인 주소 : http://15.164.153.174/user
//            Intent(mContext, 목적지) => 목적지 대응 : 기능 주소
            val urlString = "${HOST_URL}/proof_reply"
...
//                    그 가이드북에 적힌 내용을 실제로 실행해달라는 코드.
        ...
        fun postRequestLikeProofReply(context:Context, replyId: Int, handler: JsonResponseHandler?) {
...
//            로그인 주소 : http://15.164.153.174/user
//            Intent(mContext, 목적지) => 목적지 대응 : 기능 주소
            val urlString = "${HOST_URL}/like_proof_reply"

//            putExtra("이름표", 실제값) => 폼데이터.add("서버요구이름표", 실제첨부값)
            val formData = FormBody.Builder()
                .add("proof_reply_id", replyId.toString())
                .build()
